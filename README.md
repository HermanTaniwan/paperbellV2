...
Menu **Konfigurasi Printer** mendeteksi printer Windows melalui PowerShell atau printer Ubuntu melalui CUPS, sesuai sistem operasi host. Dari menu ini dapat diatur printer yang muncul di web, printer default label, serta override mapping Brother dan EPSON L3210. Semua pengaturan disimpan di MySQL dan berlaku untuk seluruh komputer pengguna.
...
- **Antrean dan alert printer**: status print worker, insiden persisten dengan diagnosis, retry manual, notifikasi browser/Windows, serta pause/resume/cancel Windows print spooler atau CUPS.
...
Cetak label/resi memakai kertas fisik 105 x 182 mm, simplex, dan berwarna agar banner pemberitahuan tetap merah. Isi resi tetap maksimal 72% dan diratakan ke kiri-atas. Jika PDF marketplace terdiri dari dua halaman, ruang kosong halaman kedua dipotong dan lanjutan isinya disusun pada lembar yang sama. Gambar pemberitahuan video unboxing ditempatkan tepat di bawah isi resi. Epson L3210 memakai margin atas aman 4 mm dan halaman driver B6 JIS 128 x 182 mm; area label 105 x 182 mm ditempatkan di tengah horizontal halaman agar sesuai dengan crop driver Epson pada kertas sempit, dengan beban raster jauh lebih kecil daripada Letter. Printer lain tetap memakai halaman custom 105 x 182 mm dengan margin atas 2 mm. Untuk Brother DCP label dipaksa melalui MP Tray (`bin=258`), sedangkan Epson WF melalui Rear Paper Feed/tray atas (`bin=261`). Saat mengirim job resi selain L3210, worker mengubah PrintTicket printer sementara ke 105 x 182 mm lalu mengembalikan konfigurasi sebelumnya sesudah job masuk spooler. Resi satu halaman memakai jalur penyiapan cepat; snapshot dan korelasi spooler sinkron yang redundan dilewati untuk semua printer, ukuran produk Brother dicache saat worker mulai agar pemeriksaan driver yang lambat tidak masuk waktu tunggu pengguna, dan durasi setiap tahap pengiriman dicatat di `storage/print-worker.log`.
...
Runbook untuk pembaruan stok Shopee dan TikTok Shop—termasuk endpoint internal, format payload, verifikasi, audit, serta batasan multi-gudang—tersedia di [docs/marketplace-stock-runbook.md](docs/marketplace-stock-runbook.md).
