...
function activeShippingTarget(PDO $db,array $holidayLookup): array {
    $timezone=new DateTimeZone(date_default_timezone_get());$today=(new DateTimeImmutable('today',$timezone))->setTime(0,0);$range=shippingDueDateRange($today,$holidayLookup);$todaySummary=shippingSummary($db,$range);$date=$today;$period='today';$summary=$todaySummary;
    if($todaySummary['total']>0&&$todaySummary['unprinted']===0){$date=nextShippingWorkday($today,$holidayLookup);$range=shippingDueDateRange($date,$holidayLookup);$summary=shippingSummary($db,$range);$period='next';$summary['todayTotal']=$todaySummary['total'];}
    return ['date'=>$date->format('Y-m-d'),'period'=>$period,'range'=>$range,'summary'=>$summary];
}
...
try {
    ...
    if ($action === 'orders') {
        ...
        $holidayLookup=array_fill_keys(storeHolidays($mysql),true);$shippingTarget=activeShippingTarget($mysql,$holidayLookup);$dueRange=$shippingTarget['range'];$shippingSummary=$shippingTarget['summary']+['period'=>$shippingTarget['period'],'date'=>$shippingTarget['date']];
        $where=[]; $params=[];
    ...
    if ($action === 'labels') {
        ...
        $stmt=$mysql->prepare("SELECT o.order_sn,o.status,o.create_time,COALESCE(oq.item_qty,0) item_qty,IFNULL(r.pdf_path,'') pdf_path,IFNULL(r.tracking_number,'') tracking_number,IFNULL(r.resi_printed,0) resi_printed,r.resi_printed_at,IFNULL(lf.status,'') label_fetch_status,IFNULL(lf.message,'') label_fetch_message,IFNULL(lf.error,'') label_fetch_error,IFNULL(lf.attempts,0) label_fetch_attempts FROM orders o LEFT JOIN order_resi r ON r.order_sn=o.order_sn LEFT JOIN label_fetch_jobs lf ON lf.order_sn=o.order_sn LEFT JOIN (SELECT order_sn,COALESCE(SUM(qty),0) item_qty FROM order_process WHERE qty>0 AND UPPER(TRIM(status)) NOT IN ('CANCELLED','CANCELED') GROUP BY order_sn) oq ON oq.order_sn=o.order_sn {$sqlWhere} ORDER BY {$labelOrderBy} LIMIT {$size} OFFSET {$offset}");$stmt->execute($params);$items=$stmt->fetchAll();foreach($items as &$row){$deadline=shippingDeadline($row['create_time'],$holidayLookup);$row['createdText']=unixText($row['create_time']);$row['shipping_deadline']=$deadline['date'];$row['shipping_due_today']=$deadline['date']===$shippingTarget['date']&&!in_array(strtoupper((string)$row['status']),['CANCELLED','CANCELED'],true);$row['item_qty']=(int)$row['item_qty'];$row['hasPdf']=$row['pdf_path']!==''&&is_file($row['pdf_path']);unset($row['pdf_path']);$row['resi_printed']=(bool)$row['resi_printed'];}respond(['items'=>$items,'total'=>$total,'page'=>$page,'pages'=>max(1,(int)ceil($total/$size)),'shippingTarget'=>['period'=>$shippingTarget['period'],'date'=>$shippingTarget['date']],'printers'=>$printing->labelPrinters(),'defaultPrinter'=>$printing->defaultLabelPrinter()]);
    }
