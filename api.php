...
try {
    ...
    if ($action === 'orders') {
        ...
        $loadShippingSummary=static function(?array $range)use($mysql):array{$summary=['total'=>0,'unprinted'=>0,'printed'=>0,'item_total'=>0,'item_printed'=>0];if($range===null)return$summary;$stmt=$mysql->prepare("SELECT COUNT(*) total,COALESCE(SUM(o.print_line_count=0 OR o.unprinted_lines>0),0) unprinted,COALESCE(SUM(o.print_item_qty),0) item_total,COALESCE(SUM(COALESCE(p.item_printed,0)),0) item_printed FROM orders o LEFT JOIN (SELECT order_sn,COALESCE(SUM(CASE WHEN printed=1 THEN qty ELSE 0 END),0) item_printed FROM order_process GROUP BY order_sn) p ON p.order_sn=o.order_sn WHERE o.order_sn NOT LIKE 'MANUAL-%' AND o.order_sn NOT LIKE 'RANDOM-%' AND UPPER(o.status) NOT IN ('CANCELLED','CANCELED') AND o.create_time>=? AND o.create_time<?");$stmt->execute($range);$row=$stmt->fetch()?:[];$summary['total']=(int)($row['total']??0);$summary['unprinted']=(int)($row['unprinted']??0);$summary['printed']=$summary['total']-$summary['unprinted'];$summary['item_total']=(int)($row['item_total']??0);$summary['item_printed']=(int)($row['item_printed']??0);return$summary;};
