...
try {
    ...
    if ($action === 'orders') {
        ...
        $orderItems=$printing->listOrderItems(array_column($items,'order_sn'));
        foreach($items as &$row){$deadline=shippingDeadline($row['create_time'],$holidayLookup);$row['createdText']=unixText($row['create_time']);$row['shipping_deadline']=$deadline['date'];$row['shipping_due_today']=$deadline['date']===$shippingTarget['date']&&!str_starts_with((string)$row['order_sn'],'MANUAL-')&&!str_starts_with((string)$row['order_sn'],'RANDOM-')&&!in_array(strtoupper((string)$row['status']),['CANCELLED','CANCELED'],true);$row['packaged']=(bool)$row['packaged'];$row['has_label_pdf']=$row['label_pdf_path']!==''&&is_file($row['label_pdf_path']);$row['resi_printed']=(bool)$row['resi_printed'];$row['loyalty']=$loyaltyByBuyer[(string)$row['buyer_username']]??null;$row['items']=$orderItems[(string)$row['order_sn']]??[];if(isset($row['item_ids'])&&is_array($row['item_ids'])){$allowed=array_fill_keys(array_map('intval',$row['item_ids']),true);$row['items']=array_values(array_filter($row['items'],fn($line)=>isset($allowed[(int)$line['id']])));}unset($row['label_pdf_path']);}
    ...
    if ($action === 'labels') {
        ...
        $stmt=$mysql->prepare("SELECT o.order_sn,o.status,o.create_time,COALESCE(oq.item_qty,0) item_qty,IFNULL(r.pdf_path,'') pdf_path,IFNULL(r.tracking_number,'') tracking_number,IFNULL(r.resi_printed,0) resi_printed,r.resi_printed_at,IFNULL(lf.status,'') label_fetch_status,IFNULL(lf.message,'') label_fetch_message,IFNULL(lf.error,'') label_fetch_error,IFNULL(lf.attempts,0) label_fetch_attempts FROM orders o LEFT JOIN order_resi r ON r.order_sn=o.order_sn LEFT JOIN label_fetch_jobs lf ON lf.order_sn=o.order_sn LEFT JOIN (SELECT order_sn,COALESCE(SUM(qty),0) item_qty FROM order_process WHERE qty>0 AND UPPER(TRIM(status)) NOT IN ('CANCELLED','CANCELED') GROUP BY order_sn) oq ON oq.order_sn=o.order_sn {$sqlWhere} ORDER BY {$labelOrderBy} LIMIT {$size} OFFSET {$offset}");$stmt->execute($params);$items=$stmt->fetchAll();foreach($items as &$row){$deadline=shippingDeadline($row['create_time'],$holidayLookup);$row['createdText']=unixText($row['create_time']);$row['shipping_deadline']=$deadline['date'];$row['shipping_due_today']=$deadline['date']===$shippingTarget['date']&&!in_array(strtoupper((string)$row['status']),['CANCELLED','CANCELED'],true);$row['item_qty']=(int)$row['item_qty'];$row['hasPdf']=$row['pdf_path']!==''&&is_file($row['pdf_path']);unset($row['pdf_path']);$row['resi_printed']=(bool)$row['resi_printed'];}respond(['items'=>$items,'total'=>$total,'page'=>$page,'pages'=>max(1,(int)ceil($total/$size)),'shippingTarget'=>['period'=>$shippingTarget['period'],'date'=>$shippingTarget['date'],'rollover'=>$shippingTarget['rollover']],'printers'=>$printing->labelPrinters(),'defaultPrinter'=>$printing->defaultLabelPrinter()]);
    }
