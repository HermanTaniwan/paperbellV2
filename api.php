...
function shippingSummary(PDO $db,?array $range): array {
    $summary=['total'=>0,'unprinted'=>0,'printed'=>0,'item_total'=>0,'item_printed'=>0];
    if($range===null)return $summary;
    $stmt=$db->prepare("SELECT COUNT(*) total,COALESCE(SUM(o.print_line_count=0 OR o.unprinted_lines>0),0) unprinted,COALESCE(SUM(o.print_item_qty),0) item_total,COALESCE(SUM(COALESCE(p.item_printed,0)),0) item_printed FROM orders o LEFT JOIN (SELECT order_sn,COALESCE(SUM(CASE WHEN printed=1 THEN qty ELSE 0 END),0) item_printed FROM order_process GROUP BY order_sn) p ON p.order_sn=o.order_sn WHERE o.order_sn NOT LIKE 'MANUAL-%' AND o.order_sn NOT LIKE 'RANDOM-%' AND UPPER(o.status) NOT IN ('CANCELLED','CANCELED') AND o.create_time>=? AND o.create_time<?");
    $stmt->execute($range);$row=$stmt->fetch()?:[];
    $summary['total']=(int)($row['total']??0);$summary['unprinted']=(int)($row['unprinted']??0);$summary['printed']=$summary['total']-$summary['unprinted'];$summary['item_total']=(int)($row['item_total']??0);$summary['item_printed']=(int)($row['item_printed']??0);
    return $summary;
}
function activeShippingTarget(PDO $db,array $holidayLookup): array {
    $timezone=new DateTimeZone(date_default_timezone_get());$today=(new DateTimeImmutable('today',$timezone))->setTime(0,0);$range=shippingDueDateRange($today,$holidayLookup);$todaySummary=shippingSummary($db,$range);$date=$today;$period='today';$rollover='';$summary=$todaySummary;
    if($range===null){$date=nextShippingWorkday($today,$holidayLookup);$range=shippingDueDateRange($date,$holidayLookup);$summary=shippingSummary($db,$range);$period='next';$rollover='non_workday';}
    elseif($todaySummary['total']>0&&$todaySummary['unprinted']===0){$date=nextShippingWorkday($today,$holidayLookup);$range=shippingDueDateRange($date,$holidayLookup);$summary=shippingSummary($db,$range);$period='next';$rollover='complete';$summary['todayTotal']=$todaySummary['total'];}
    return ['date'=>$date->format('Y-m-d'),'period'=>$period,'rollover'=>$rollover,'range'=>$range,'summary'=>$summary];
}
...
function orderCourierFromRawJson(string $rawJson): string {
    $raw=json_decode($rawJson,true);if(!is_array($raw))return '';
    $keys=['shipping_carrier','shipping_provider_name','shipping_provider','courier_name','courier','logistics_service_name','logistics_service','delivery_service_name','delivery_service'];
    $find=function(mixed $value)use(&$find,$keys):string{if(!is_array($value))return '';foreach($keys as $key)if(isset($value[$key])&&(is_string($value[$key])||is_numeric($value[$key]))&&trim((string)$value[$key])!=='')return trim((string)$value[$key]);foreach($value as $child)if(is_array($child)&&($found=$find($child))!=='')return $found;return '';};
    return mb_substr($find($raw),0,100);
}
function orderCourierData(PDO $db): array {
    $byOrder=[];$names=[];$stmt=$db->query("SELECT order_sn,raw_json FROM orders WHERE order_sn NOT LIKE 'MANUAL-%' AND order_sn NOT LIKE 'RANDOM-%'");
    foreach($stmt->fetchAll() as $row){$courier=orderCourierFromRawJson((string)$row['raw_json']);if($courier==='')continue;$byOrder[(string)$row['order_sn']]=$courier;$names[mb_strtolower($courier)]=$courier;}
    natcasesort($names);return ['byOrder'=>$byOrder,'names'=>array_values($names)];
}
...
try {
    ...
    if ($action === 'orders') {
        ...
        if ($q!=='') { $where[]='(o.order_sn LIKE ? OR o.buyer_username LIKE ? OR r.tracking_number LIKE ? OR EXISTS(SELECT 1 FROM order_process op WHERE op.order_sn=o.order_sn AND (op.item_name LIKE ? OR op.model_name LIKE ? OR op.model_sku LIKE ? OR op.item_sku LIKE ?)))'; $term="%{$q}%"; $params=array_merge($params,array_fill(0,7,$term)); }
    ...
    if ($action === 'customer_history') {$buyer=trim((string)($_GET['buyer']??''));if($buyer==='')respond(['error'=>'Nama customer wajib diisi.'],422);$since=strtotime('-1 year');$stmt=$mysql->prepare('SELECT o.order_sn,o.create_time,op.item_name,op.model_name,op.qty FROM orders o JOIN order_process op ON op.order_sn=o.order_sn WHERE o.buyer_username=? AND o.create_time>=? ORDER BY o.create_time DESC,op.id');$stmt->execute([$buyer,$since]);$rows=$stmt->fetchAll();$orders=[];$totalQty=0;foreach($rows as &$row){$row['createdText']=unixText($row['create_time']);$orders[$row['order_sn']]=true;$totalQty+=(int)$row['qty'];}respond(['buyer'=>$buyer,'summary'=>['orders'=>count($orders),'lines'=>count($rows),'qty'=>$totalQty,'period'=>'1 tahun terakhir'],'loyalty'=>$customerLoyalty->forBuyer($buyer),'items'=>$rows]);}

    if ($action === 'labels') {
        $q=trim((string)($_GET['q']??''));$filter=$_GET['filter']??'all';$dueToday=(string)($_GET['due_today']??'')==='1';$page=max(1,(int)($_GET['page']??1));$size=30;$offset=($page-1)*$size;$where=["o.order_sn NOT LIKE 'MANUAL-%'","o.order_sn NOT LIKE 'RANDOM-%'"];$params=[];
        $holidayLookup=array_fill_keys(storeHolidays($mysql),true);$shippingTarget=activeShippingTarget($mysql,$holidayLookup);
        $courierData=orderCourierData($mysql);$requestedCouriers=array_values(array_filter(array_map('trim',explode(',',(string)($_GET['couriers']??'')))));if($requestedCouriers){$wanted=array_fill_keys(array_map('mb_strtolower',$requestedCouriers),true);$courierOrderSns=array_keys(array_filter($courierData['byOrder'],fn($courier)=>isset($wanted[mb_strtolower($courier)])));if(!$courierOrderSns)$where[]='1=0';else{$marks=implode(',',array_fill(0,count($courierOrderSns),'?'));$where[]="o.order_sn IN ({$marks})";$params=array_merge($params,$courierOrderSns);}}
        if($q!==''){$where[]='(o.order_sn LIKE ? OR r.tracking_number LIKE ?)';$params[]="%{$q}%";$params[]="%{$q}%";}if($filter==='unprinted')$where[]="IFNULL(r.resi_printed,0)=0 AND ((o.order_sn LIKE 'TIKTOK:%' AND UPPER(o.status) IN ('AWAITING_SHIPMENT','AWAITING_COLLECTION')) OR (o.order_sn NOT LIKE 'TIKTOK:%' AND UPPER(o.status) IN ('PROCESSED','READY_TO_SHIP')))";if($dueToday){$dueRange=$shippingTarget['range'];if($dueRange===null)$where[]='1=0';else{$where[]="o.create_time>=? AND o.create_time<? AND ((o.order_sn LIKE 'TIKTOK:%' AND UPPER(o.status) IN ('AWAITING_SHIPMENT','AWAITING_COLLECTION')) OR (o.order_sn NOT LIKE 'TIKTOK:%' AND UPPER(o.status) IN ('PROCESSED','READY_TO_SHIP')))";$params[]=$dueRange[0];$params[]=$dueRange[1];}}if($filter==='printed')$where[]='r.resi_printed=1';if($filter==='cancelled')$where[]="UPPER(o.status)='CANCELLED'";$sqlWhere=$where?'WHERE '.implode(' AND ',$where):'';
        $count=$mysql->prepare("SELECT COUNT(*) FROM orders o LEFT JOIN order_resi r ON r.order_sn=o.order_sn {$sqlWhere}");$count->execute($params);$total=(int)$count->fetchColumn();
        $labelOrderBy=$filter==='printed'?'COALESCE(r.resi_printed_at,0) DESC,o.create_time DESC':'o.create_time DESC';
        $stmt=$mysql->prepare("SELECT o.order_sn,o.status,o.create_time,COALESCE(oq.item_qty,0) item_qty,IFNULL(r.pdf_path,'') pdf_path,IFNULL(r.tracking_number,'') tracking_number,IFNULL(r.resi_printed,0) resi_printed,r.resi_printed_at,IFNULL(lf.status,'') label_fetch_status,IFNULL(lf.message,'') label_fetch_message,IFNULL(lf.error,'') label_fetch_error,IFNULL(lf.attempts,0) label_fetch_attempts FROM orders o LEFT JOIN order_resi r ON r.order_sn=o.order_sn LEFT JOIN label_fetch_jobs lf ON lf.order_sn=o.order_sn LEFT JOIN (SELECT order_sn,COALESCE(SUM(qty),0) item_qty FROM order_process WHERE qty>0 AND UPPER(TRIM(status)) NOT IN ('CANCELLED','CANCELED') GROUP BY order_sn) oq ON oq.order_sn=o.order_sn {$sqlWhere} ORDER BY {$labelOrderBy} LIMIT {$size} OFFSET {$offset}");$stmt->execute($params);$items=$stmt->fetchAll();foreach($items as &$row){$deadline=shippingDeadline($row['create_time'],$holidayLookup);$row['createdText']=unixText($row['create_time']);$row['shipping_deadline']=$deadline['date'];$row['shipping_due_today']=$deadline['date']===$shippingTarget['date']&&!in_array(strtoupper((string)$row['status']),['CANCELLED','CANCELED'],true);$row['item_qty']=(int)$row['item_qty'];$row['hasPdf']=$row['pdf_path']!==''&&is_file($row['pdf_path']);unset($row['pdf_path']);$row['resi_printed']=(bool)$row['resi_printed'];}respond(['items'=>$items,'total'=>$total,'page'=>$page,'pages'=>max(1,(int)ceil($total/$size)),'shippingTarget'=>['period'=>$shippingTarget['period'],'date'=>$shippingTarget['date'],'rollover'=>$shippingTarget['rollover']],'couriers'=>$courierData['names'],'printers'=>$printing->labelPrinters(),'defaultPrinter'=>$printing->defaultLabelPrinter()]);
    }
