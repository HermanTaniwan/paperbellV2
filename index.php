...
<small>{{ number(dashboard.inventory.items) }} jenis produk</small>
      ...
      <section v-if="view==='profit'" class="content shopee-page" v-cloak>
        ...
        <article class="panel shopee-card"><div class="shopee-card-head"><div><h4>HPP per SKU</h4><p>HPP baru dikunci pada order yang masuk setelah HPP disimpan.</p></div></div><div class="shopee-table-wrap"><table class="shopee-product-table profit-cost-table"><thead><tr><th>Produk / SKU</th><th>HPP per unit</th><th></th></tr></thead><tbody><tr v-for="item in profitCosts.items" :key="item.sku_id"><td><b>{{item.product_name}}</b><small>{{item.sku_id}} · {{item.variation_name}}</small></td><td><input v-model.number="profitCostDrafts[item.sku_id]" min="0" step="1000" type="number"></td><td><button type="button" class="ghost" :disabled="profitSaving" @click="saveProfitCost(item.sku_id)">Simpan</button></td></tr></tbody></table></div></article>
