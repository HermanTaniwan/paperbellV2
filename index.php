...
<?= htmlspecialchars($config['app']['name']) ?>
  ...
  <link rel="stylesheet" href="assets/shopee-insights.css?v=10">
...
<small>{{ number(dashboard.inventory.items) }} jenis produk</small>
      ...
      <section v-if="view==='profit'" class="content shopee-page" v-cloak>
        <article class="panel shopee-page-hero"><div><span class="eyebrow">PROFIT & LOSS</span><h2>Laba rugi bulanan</h2><p>Berbasis bulan order dibuat: omzet dikurangi HPP, fee marketplace, dan biaya operasional.</p></div><label class="profit-month">Bulan<input v-model="profitMonth" type="month" :max="analyticsMonthMax()" @change="loadProfitLoss"></label></article>
        <template v-if="profitLoss"><section class="shopee-kpis shopee-page-kpis"><article class="shopee-kpi"><span>Omzet {{profitLoss.label}}</span><strong>{{currency((profitLoss.revenue?.shopee||0)+(profitLoss.revenue?.tiktok||0))}}</strong><small>Shopee {{currency(profitLoss.revenue?.shopee)}} · TikTok {{currency(profitLoss.revenue?.tiktok)}}</small></article><article class="shopee-kpi"><span>Laba bersih</span><strong>{{currency(profitLoss.netProfit)}}</strong><small>Setelah HPP, fee, dan biaya manual</small></article><article class="shopee-kpi"><span>Margin laba bersih</span><strong>{{number(profitLoss.margin)}}%</strong><small>Dari total omzet</small></article><article class="shopee-kpi"><span>Payout escrow Shopee</span><strong>{{currency(profitLoss.payout)}}</strong><small>Cashflow, bukan pengurang laba</small></article></section>
        <p v-if="profitLoss.coverage?.missingUnits" class="profit-notice warning">{{number(profitLoss.coverage.missingUnits)}} unit belum memiliki HPP. Nilainya belum masuk biaya; isi HPP SKU di bawah.</p><p v-if="profitLoss.coverage?.estimatedUnits" class="profit-notice">{{number(profitLoss.coverage.estimatedUnits)}} unit historis memakai HPP aktif saat ini (estimasi).</p><p v-if="profitLoss.coverage?.escrowPercent<100" class="profit-notice warning">Escrow Shopee baru {{number(profitLoss.coverage.escrowPercent)}}% tersinkron. Fee Shopee mungkin belum lengkap.</p>
        <section class="shopee-page-grid"><article class="panel shopee-card"><div class="shopee-card-head"><div><h4>Rincian P&L</h4><p>{{profitLoss.label}} · payout ditampilkan terpisah sebagai cashflow.</p></div><button type="button" class="ghost" @click="syncProfitEscrow" :disabled="financeSyncing">{{financeSyncing?'Sync...':'Sync escrow'}}</button></div><div class="profit-lines"><div><span>Omzet Shopee</span><strong>{{currency(profitLoss.revenue?.shopee)}}</strong></div><div><span>Omzet TikTok</span><strong>{{currency(profitLoss.revenue?.tiktok)}}</strong></div><div class="deduction"><span>HPP produk</span><strong>− {{currency(profitLoss.cogs)}}</strong></div><div class="deduction"><span>Fee marketplace Shopee</span><strong>− {{currency(profitLoss.marketplaceFees)}}</strong></div><div class="deduction"><span>Biaya operasional manual</span><strong>− {{currency(profitExpenseTotal())}}</strong></div><div class="total"><span>Laba bersih</span><strong>{{currency(profitLoss.netProfit)}}</strong></div></div></article>
        <article class="panel shopee-card"><div class="shopee-card-head"><div><h4>Biaya bulanan</h4><p>Fee Shopee otomatis dari escrow dan tidak diisi di sini.</p></div></div><form class="profit-expenses" @submit.prevent="saveProfitExpenses"><label v-for="item in profitExpenseRows()" :key="item.key"><span>{{item.label}}</span><input v-model.number="profitExpenses[item.key]" min="0" step="1000" type="number"></label><button type="submit" :disabled="profitSaving">{{profitSaving?'Menyimpan...':'Simpan biaya'}}</button></form></article></section>
        <section class="shopee-page-grid"><article class="panel shopee-card"><div class="shopee-card-head"><div><h4>HPP kategori produk</h4><p>Jurnal, loose leaf, binder, dan sampul memakai HPP kategori.</p></div></div><div class="shopee-table-wrap"><table class="shopee-product-table profit-cost-table"><thead><tr><th>Kategori</th><th>HPP per unit</th><th></th></tr></thead><tbody><tr v-for="item in profitCosts.categories" :key="item.key"><td><b>{{item.label}}</b></td><td><input v-model.number="profitCostDrafts[item.key]" min="0" step="1000" type="number"></td><td><button type="button" class="ghost" :disabled="profitSaving" @click="saveProfitCost(item.key)">Simpan</button></td></tr></tbody></table></div></article><article class="panel shopee-card"><div class="shopee-card-head"><div><h4>HPP sticker per SKU</h4><p>Setiap desain sticker dapat memiliki modal sendiri.</p></div></div><div class="shopee-table-wrap"><table class="shopee-product-table profit-cost-table"><thead><tr><th>Sticker / SKU</th><th>HPP per unit</th><th></th></tr></thead><tbody><tr v-for="item in profitCosts.stickers" :key="item.key"><td><b>{{item.product_name}}</b><small>{{item.sku_id}} · {{item.variation_name}}</small></td><td><input v-model.number="profitCostDrafts[item.key]" min="0" step="1000" type="number"></td><td><button type="button" class="ghost" :disabled="profitSaving" @click="saveProfitCost(item.key)">Simpan</button></td></tr></tbody></table></div></article></section>
        <article class="panel shopee-card"><div class="shopee-card-head"><div><h4>12 bulan terakhir</h4><p>Ringkasan omzet, total biaya, dan laba bersih per bulan (terbaru lebih dulu).</p></div></div><div class="shopee-table-wrap"><table class="shopee-product-table"><thead><tr><th>Bulan</th><th>Omzet</th><th>Total biaya</th><th>Laba bersih</th><th>Margin</th></tr></thead><tbody><tr v-for="item in profitLoss.history" :key="item.month"><td><b>{{item.label}}</b></td><td>{{currency(item.revenue)}}</td><td>{{currency(item.totalCost)}}</td><td>{{currency(item.netProfit)}}</td><td>{{number(item.margin)}}%</td></tr></tbody></table></div></article></template>
        <div v-else-if="profitLoading" class="analytics-empty">Memuat laporan P&L…</div>
      </section>

      <section v-if="view==='growth'" class="content shopee-page" v-cloak>
        <template v-if="shopeeStats"><article class="panel shopee-page-hero"><div><span class="eyebrow">IKLAN & PERTUMBUHAN</span><h2>Efektivitas akuisisi</h2><p>{{shopeeGrowth?.periodLabel||shopeeStats.meta.latestLabel}} · attribution antar-channel dapat saling tumpang tindih.</p></div></article>
        <div class="shopee-date-controls shopee-growth-date-controls"><label><span>Dari</span><input v-model="shopeeGrowthFrom" type="date" @click="openDatePicker($event)" :min="shopeeGrowth?.minDate||shopeeStats.meta.periodStart" :max="shopeeGrowth?.maxDate||shopeeStats.meta.periodEnd"></label><label><span>Sampai</span><input v-model="shopeeGrowthTo" type="date" @click="openDatePicker($event)" :min="shopeeGrowth?.minDate||shopeeStats.meta.periodStart" :max="shopeeGrowth?.maxDate||shopeeStats.meta.periodEnd"></label><button type="button" :disabled="shopeeGrowthLoading" @click="loadShopeeGrowth()">{{shopeeGrowthLoading?'Memuat...':'Terapkan'}}</button></div>
        <section v-if="shopeeGrowth" class="shopee-kpis shopee-page-kpis"><article class="shopee-kpi"><span>Omzet iklan</span><strong>{{currency(shopeeGrowth.ads.sales)}}</strong><small>{{shopeeGrowth.ads.name}}</small></article><article class="shopee-kpi"><span>Belanja iklan</span><strong>{{currency(shopeeGrowth.ads.spend)}}</strong><small>{{number(shopeeGrowth.ads.impressions)}} impresi</small></article><article class="shopee-kpi"><span>ROAS</span><strong>{{number(shopeeGrowth.ads.roas)}}×</strong><small>{{number(shopeeGrowth.ads.orders)}} order atribusi</small></article><article class="shopee-kpi"><span>Konversi iklan</span><strong>{{shopeePercent(shopeeGrowth.ads.conversion,2)}}</strong><small>Berbasis laporan iklan</small></article></section>
        <small v-if="shopeeGrowth?.hasAdsData" class="shopee-growth-method">Agregat iklan hanya mencakup bulan dengan belanja iklan tercatat.</small>
        <article class="panel shopee-card shopee-growth-chart"><div class="shopee-card-head"><div><h4>Performa harian</h4><p>Omzet dan pengunjung mengikuti rentang tanggal di atas. Data iklan tersedia per bulan, sehingga tidak diplot harian.</p></div></div><div v-if="shopeeComparison" class="shopee-comparison-summary"><div><span>Total pendapatan</span><strong>{{currency(shopeeComparison.summary.sales)}}</strong></div><div><span>Akumulasi pengunjung</span><strong>{{number(shopeeComparison.summary.visitors)}}</strong></div><div><span>Belanja iklan</span><strong>{{currency(shopeeGrowth?.ads.spend)}}</strong></div></div><div v-if="shopeeComparison" class="shopee-comparison-legend"><span class="sales">Pendapatan</span><span class="visitors">Pengunjung</span><small>Tiap hari</small></div><div v-if="shopeeComparison?.items?.length" class="shopee-comparison-scroll"><div class="shopee-comparison-canvas"><svg viewBox="0 0 860 290" role="img" aria-label="Grafik harian pendapatan dan pengunjung Shopee"><g class="shopee-comparison-grid"><g v-for="tick in shopeeChartTicks('sales')" :key="'growth-sales-'+tick.ratio"><line :x1="shopeeChartLeft()" :y1="tick.y" :x2="shopeeChartRight()" :y2="tick.y"></line><text :x="shopeeChartLeft()-9" :y="tick.y+3" text-anchor="end">{{shopeeChartCurrencyTick(tick.value)}}</text></g></g><g class="shopee-comparison-right-axis"><text v-for="tick in shopeeChartTicks('visitors')" :key="'growth-visitor-'+tick.ratio" :x="shopeeChartRight()+9" :y="tick.y+3">{{number(Math.round(tick.value))}}</text></g><line class="shopee-comparison-axis" :x1="shopeeChartLeft()" y1="225" :x2="shopeeChartRight()" y2="225"></line><g class="shopee-comparison-bars"><rect v-for="(item,index) in shopeeComparison.items" :key="'growth-bar-'+item.date" :x="shopeeChartX(index)-shopeeChartBarWidth()/2" :y="shopeeChartSalesY(item.sales)" :width="shopeeChartBarWidth()" :height="225-shopeeChartSalesY(item.sales)" :rx="Math.min(4,shopeeChartBarWidth()/2)"><title>{{item.fullLabel}}: {{shopeeComparisonDetail(item)}}</title></rect></g><polyline class="shopee-comparison-line" :points="shopeeChartVisitorPoints()"></polyline><g v-if="shopeeComparison.items.length<=90" class="shopee-comparison-points"><circle v-for="(item,index) in shopeeComparison.items" :key="'growth-point-'+item.date" :cx="shopeeChartX(index)" :cy="shopeeChartVisitorY(item.visitors)" r="4"><title>{{item.fullLabel}}: {{number(item.visitors)}} pengunjung</title></circle></g><g class="shopee-comparison-x-axis"><g v-for="tick in shopeeChartDateTicks()" :key="'growth-date-'+tick.index"><line :x1="shopeeChartX(tick.index)" y1="225" :x2="shopeeChartX(tick.index)" y2="230"></line><text :x="shopeeChartX(tick.index)" y="244" text-anchor="middle">{{tick.label}}</text></g></g><g class="shopee-comparison-hover"><rect v-for="(item,index) in shopeeComparison.items" :key="'growth-zone-'+item.date" :x="shopeeChartZoneX(index)" y="28" :width="shopeeChartZoneWidth()" height="197" @mouseenter="shopeeComparisonSelected=item" @click="shopeeComparisonSelected=item"><title>{{item.fullLabel}}: {{shopeeComparisonDetail(item)}}</title></rect></g><text class="shopee-comparison-axis-title" :x="shopeeChartLeft()" y="17">Pendapatan</text><text class="shopee-comparison-axis-title visitors" :x="shopeeChartRight()" y="17" text-anchor="end">Pengunjung</text></svg><div v-if="shopeeComparisonSelected" class="shopee-comparison-tooltip" :style="{left:shopeeChartTooltipPercent(shopeeComparisonSelected)+'%'}"><b>{{shopeeComparisonSelected.fullLabel}}</b><span>{{currency(shopeeComparisonSelected.sales)}} pendapatan</span><span>{{number(shopeeComparisonSelected.visitors)}} pengunjung</span><small>{{currency(shopeeComparisonSelected.salesPerVisitor)}} / pengunjung</small></div></div></div><div v-else-if="shopeeComparisonLoading" class="shopee-comparison-empty">Memuat grafik harian...</div><div v-else class="shopee-comparison-empty">Tidak ada data harian pada rentang ini.</div></article>
        <p v-if="shopeeGrowth&&!shopeeGrowth.hasAdsData" class="shopee-growth-notice">Belum ada laporan iklan untuk rentang ini. Data funnel toko tetap tersedia di bawah.</p>
        <section v-if="shopeeGrowth" class="shopee-page-grid"><article class="panel shopee-card"><div class="shopee-card-head"><div><h4>Atribusi channel</h4><p>Akumulasi kontribusi penjualan pada rentang terpilih.</p></div></div><div v-if="shopeeGrowth.attribution.length" class="attribution-list"><div v-for="channel in shopeeGrowth.attribution" :key="channel.name"><span>{{channel.name}}</span><strong>{{currency(channel.sales)}}</strong></div></div><p v-else class="shopee-growth-empty">Belum ada data atribusi channel pada rentang ini.</p></article><article class="panel shopee-card"><div class="shopee-card-head"><div><h4>Funnel toko</h4><p>Traffic dan kualitas konversi pada rentang terpilih.</p></div></div><div class="cashflow-summary"><div><span>Pengunjung</span><strong>{{number(shopeeGrowth.funnel.visitors)}}</strong></div><div><span>Klik</span><strong>{{number(shopeeGrowth.funnel.clicks)}}</strong></div><div><span>Konversi</span><strong>{{shopeePercent(shopeeGrowth.funnel.conversion,2)}}</strong></div></div></article></section></template>
      </section>
...
<div class="manual-order-footer"><button class="ghost" @click="randomPrint.open=false">Batal</button><button :disabled="randomPrint.printing" @click="generateRandomFromPopup">{{randomPrint.printing?'Membuat PDF…':'Buat Random Pages'}}</button></div>
      ...
      <section v-if="view==='stock-management'" class="content stock-management-page">
        ...
        <div v-if="stockManagementAdd.open" class="modal-backdrop" @click.self="stockManagementAdd.open=false"><article class="panel sm-modal"><div class="panel-head"><div><h3>Tambah variasi terkelola</h3><p>Pilih satu variasi Shopee dan satu variasi TikTok / Tokopedia yang merupakan produk yang sama.</p></div><button class="icon-button" @click="stockManagementAdd.open=false">×</button></div>
          <div class="sm-picker-grid"><section><b>1. Variasi Shopee</b><input v-model="stockManagementAdd.shopeeQuery" @input="searchStockManagement('shopee')" placeholder="Cari produk atau variasi"><p v-if="stockManagementAdd.shopeeLoading">Mencari…</p><div class="sm-results"><button v-for="item in stockManagementAdd.shopeeItems" :key="item.item_id+'-'+item.model_id" class="ghost" :class="{selected:stockManagementAdd.shopeeSelected&&stockManagementAdd.shopeeSelected.item_id===item.item_id&&stockManagementAdd.shopeeSelected.model_id===item.model_id}" @click="stockManagementAdd.shopeeSelected=item"><b>{{item.item_name}}</b><small>{{item.model_name||'Tanpa variasi'}} · SKU: {{item.model_sku||'—'}} · Stok {{item.stock===null?'—':item.stock}}</small></button></div></section>
            <section><b>2. Variasi TikTok / Tokopedia</b><input v-model="stockManagementAdd.tiktokQuery" @input="searchStockManagement('tiktok')" placeholder="Cari produk atau variasi"><p v-if="stockManagementAdd.tiktokLoading">Mencari…</p><div class="sm-results"><button v-for="item in stockManagementAdd.tiktokItems" :key="item.product_id+'-'+item.sku_id" class="ghost" :class="{selected:stockManagementAdd.tiktokSelected&&stockManagementAdd.tiktokSelected.product_id===item.product_id&&stockManagementAdd.tiktokSelected.sku_id===item.sku_id}" @click="stockManagementAdd.tiktokSelected=item"><b>{{item.title}}</b><small>{{item.variant_name||'Tanpa variasi'}} · SKU: {{item.seller_sku||'—'}} · Stok {{item.stock===null?'—':item.stock}}</small></button></div></section></div>
          <p class="sm-note">Satu kali simpan menambahkan satu pasangan variasi. Pastikan warna atau jenis variasinya sesuai sebelum menyimpan.</p><div class="sm-modal-actions"><button class="ghost" @click="stockManagementAdd.open=false">Batal</button><button @click="addStockManagement" :disabled="stockManagementAdd.saving||!stockManagementAdd.shopeeSelected||!stockManagementAdd.tiktokSelected">{{stockManagementAdd.saving?'Menyimpan…':'Tambahkan variasi'}}</button></div>
        </article></div>
      </section>
...
<script src="assets/app.js?v=151">
</script>
</body>
</html>
