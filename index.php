...
<dd>{{ dashboard?.queued || 0 }}</dd>
      ...
      <section v-if="view==='growth'" class="content shopee-page" v-cloak>
        ...
        <div class="shopee-date-controls"><label><span>Dari</span><input v-model="shopeeGrowthFrom" type="date" :min="shopeeGrowth?.minDate||shopeeStats.meta.periodStart" :max="shopeeGrowth?.maxDate||shopeeStats.meta.periodEnd"></label><label><span>Sampai</span><input v-model="shopeeGrowthTo" type="date" :min="shopeeGrowth?.minDate||shopeeStats.meta.periodStart" :max="shopeeGrowth?.maxDate||shopeeStats.meta.periodEnd"></label><button type="button" :disabled="shopeeGrowthLoading" @click="loadShopeeGrowth()">{{shopeeGrowthLoading?'Memuat...':'Terapkan'}}</button></div>
