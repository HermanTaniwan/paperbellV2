...
<small>{{ number(dashboard.inventory.items) }} jenis produk</small>
      ...
      <section v-if="view==='profit'" class="content shopee-page" v-cloak>
        ...
        <article class="panel shopee-card"><div class="shopee-card-head"><div><h4>12 bulan terakhir</h4><p>Ringkasan omzet dan laba bersih per bulan.</p></div></div><div class="shopee-table-wrap"><table class="shopee-product-table"><thead><tr><th>Bulan</th><th>Omzet</th><th>Laba bersih</th><th>Margin</th></tr></thead><tbody><tr v-for="item in profitLoss.history" :key="item.month"><td><b>{{item.label}}</b></td><td>{{currency(item.revenue)}}</td><td>{{currency(item.netProfit)}}</td><td>{{number(item.margin)}}%</td></tr></tbody></table></div></article></template>
