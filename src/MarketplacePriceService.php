...
final class MarketplacePriceService
{
    ...
    public function tiktokCatalogSample(string $query, int $limit = 10): array
    {
        ...
        $items=[];foreach(($json['data']['products']??[]) as $row){$title=(string)($row['title']??$row['name']??'');if(!str_contains(mb_strtolower($title),mb_strtolower($query)))continue;$id=(string)($row['id']??'');if($id==='')continue;$response=$this->tiktok('GET','/product/202309/products/'.rawurlencode($id),[],null,$auth);$detail=$response['data']['product']??$response['data']??[];$items[]=['id'=>$id,'title'=>$title,'keys'=>array_keys($detail),'category_id'=>(string)($detail['category_id']??''),'category_version'=>(string)($detail['category_version']??''),'product_attributes'=>$detail['product_attributes']??[],'package_weight'=>$detail['package_weight']??[],'package_dimensions'=>$detail['package_dimensions']??[],'is_cod_allowed'=>(bool)($detail['is_cod_allowed']??false),'sku_count'=>count($detail['skus']??[])];}
        return ['items'=>$items];
    }
