...
final class ProfitLossService
{
    ...
    public function dashboard(DateTimeImmutable $month): array
    {
        ...
        $fee=$this->db->prepare('SELECT COUNT(*) orders,COALESCE(SUM(total_marketplace_fee),0) fees,COALESCE(SUM(payout_amount),0) payout FROM shopee_escrow_details WHERE order_create_time>=? AND order_create_time<?');$fee->execute([$start,$end]);$escrow=$fee->fetch()?:[];$shopeeOrders=count(array_filter($orderRows,fn($row)=>$row['marketplace']==='shopee'));
